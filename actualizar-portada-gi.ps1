...
$ruta = Join-Path (Get-Location) "app\page.tsx"

if (-not (Test-Path -LiteralPath $ruta)) {
  throw "No se encontro app\page.tsx. Ejecuta este archivo desde la carpeta principal del proyecto."
}

$marca = Get-Date -Format "yyyyMMdd-HHmmss"
$respaldo = "$ruta.respaldo-$marca"
Copy-Item -LiteralPath $ruta -Destination $respaldo
    ...
    <main className="min-h-screen bg-[#f2f6f8] pb-24 text-slate-900 lg:pb-8">
      <header className="bg-[#0f2b46] text-white">
        <div className="mx-auto max-w-7xl px-5 pb-20 pt-6 md:px-8 md:pb-24 md:pt-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-300">Cultura · Olavarría</p>
              </div>
            </div>
          ...
          <div className="mt-9">
            <p className="text-sm font-medium text-cyan-300">Gestión Institucional</p>
            <h1 className="mt-1 text-3xl font-bold tracking-tight md:text-4xl">Hola, Hernán</h1>
        ...
        {proximoEvento && (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Próximo en agenda</p>
              <p className="truncate font-semibold">{proximoEvento.nombre}</p>
              <p className="mt-0.5 flex items-center gap-1.5 text-xs text-slate-500">
                <Clock3 size={13} />
                {proximoEvento.fecha || "Sin fecha"}
                {proximoEvento.hora ? ` · ${proximoEvento.hora.slice(0, 5)}` : ""}
              </p>
            </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 line-clamp-2 text-sm text-slate-500">
                        {dependencia.area || "Gestión de la dependencia"}
                      </p>
                    </div>
...
Write-Host ""
Write-Host "Portada de GI actualizada correctamente." -ForegroundColor Green

...
export default function PersonalPage() {
  ...
  const resultado = useMemo(() => {
    ...
    return personas.filter((persona) => {
      ...
      const coincideArea = area === "Todas" || persona.area === area;
          ...
          <section className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {resultado.map((persona) => (
              <a
                ...
                className="group block rounded-2xl border border-slate-400 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <ChevronRight
                        size={18}
                        className="shrink-0 text-slate-500 transition group-hover:translate-x-1 group-hover:text-sky-700"
                      />
                    </div>
